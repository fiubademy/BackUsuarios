...
#@pytest.mark.asyncio
async def run_post():
    return await ApiCalls.createUser(username='Nombre_test_123', email='asdasd@qwe.com', password='123456789')

#@pytest.mark.asyncio
async def run_post_same_uname():
    return await ApiCalls.createUser(username='Nombre_test_123', email='as@qwe.com', password='123456789')
...
#@pytest.mark.asyncio
async def loginGoogle(idGoogle, username, email):
    return await ApiCalls.loginGoogle(idGoogle = idGoogle, username=username, email=email)
...
def test_change_password_correctly():
    user_id = asyncio.run(run_post())["user_id"]
    user_obtained = asyncio.run(run_get_by_id(user_id))
    assert asyncio.run(change_password(user_id, '123456789', '12345678')).status_code == status.HTTP_202_ACCEPTED
    asyncio.run(run_delete(user_id))
...
def test_change_password_with_wrong_old_password_fails():
    user_id = asyncio.run(run_post())["user_id"]
    user_obtained = asyncio.run(run_get_by_id(user_id))
    assert asyncio.run(change_password(user_id, '1234567890', '123456789')).status_code == status.HTTP_400_BAD_REQUEST
    asyncio.run(run_delete(user_id))
...
def test_recover_password():
    ...
    assert asyncio.run(recover_password('123456789', token)).status_code == status.HTTP_202_ACCEPTED
    asyncio.run(run_delete(user_id))
...
def test_recover_password_without_existing_token():
    user_id = asyncio.run(run_post())["user_id"]
    user_obtained = asyncio.run(run_get_by_id(user_id))
    assert asyncio.run(recover_password('123456789', 'unexistentToken')).status_code == status.HTTP_404_NOT_FOUND
    asyncio.run(run_delete(user_id))
...
def test_correct_login():
    user_id = asyncio.run(run_post())["user_id"]
    user_obtained = asyncio.run(run_get_by_id(user_id))
    assert asyncio.run(login(user_obtained['email'], '123456789')).status_code == status.HTTP_202_ACCEPTED
    asyncio.run(run_delete(user_id))
...
def test_google_signup_in_login():
    retornoLogin = asyncio.run(loginGoogle('idNoExistenteGoogle', 'Roberto Suarez', 'email@gmail.com'))
...
def test_google_login_correctly():
    ...
    assert retornoLogin.status_code == status.HTTP_201_CREATED
    retornoLogin = asyncio.run(loginGoogle('idNoExistenteGoogle', 'Roberto Suarez', 'email@gmail.com'))
    assert retornoLogin.status_code == status.HTTP_202_ACCEPTED
    asyncio.run(deleteGoogle('idNoExistenteGoogle'))
...
def test_google_login_wrong_username():
    retornoLogin = asyncio.run(loginGoogle('idNoExistenteGoogle', 'Roberto Suarez', 'email@gmail.com'))
    assert retornoLogin.status_code == status.HTTP_201_CREATED
    retornoLogin = asyncio.run(loginGoogle('idNoExistenteGoogle', 'Roberto Suez', 'email@gmail.com'))
